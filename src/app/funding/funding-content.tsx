    ...
    return (
      <PageTransition>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
          {[1,2,3,4,5,6].map(i => (
            <Card key={i} className="p-4 animate-pulse"><div className="h-16 bg-white/[0.04] rounded" /></Card>
          ))}
        </div>
      </PageTransition>
    );
              ...
              <CardContent>
                <div className="space-y-2">
                  {FUNDING_PIPELINE_STAGES.map((stage) => {
                    const clients = fundingClientsWithApps.filter(c => c.stage === stage.id);
                      ...
                      <div key={stage.id}>
                        <div className="flex items-center gap-3 py-1.5">
                          ...
                          <div className="flex-1 h-6 rounded-lg bg-white/[0.03] overflow-hidden relative">
                            <div className="h-full rounded-lg bg-gradient-to-r from-emerald-500/20 to-cyan-500/30 transition-all duration-500" style={{ width: `${Math.max(width, count > 0 ? 8 : 0)}%` }} />
                                ...
                                <div className="flex items-center gap-3">
                                  <div className="flex-1">
                                    ...
                                    <div className="h-1.5 rounded-full bg-white/[0.04] overflow-hidden">
                                      <div className="h-full rounded-full bg-gradient-to-r from-emerald-500 to-cyan-400 transition-all" style={{ width: `${Math.min((client.approvedSoFar / client.targetFunding) * 100, 100)}%` }} />
                                    </div>
                                  </div>
                                </div>
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {pendingApps.map((app) => (
                        <tr key={app._id} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
              ...
              <CardContent>
                <div className="space-y-2">
                  {stats.recentWins.slice(0, 8).map((client: { _id: string; name: string; businessName: string; approvedSoFar: number }, i: number) => (
                    <div key={client._id} className="flex items-center gap-3 py-2 border-b border-white/[0.04] last:border-0">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="p-3 rounded-xl bg-emerald-500/[0.06] border border-emerald-500/[0.1]"><span className="text-[10px] text-white/30 uppercase tracking-wider">Total Revenue (10% fee)</span><p className="text-xl font-semibold text-emerald-400 mt-1">{formatCurrency(stats.totalRevenue)}</p></div>
                  <div className="p-3 rounded-xl bg-cyan-500/[0.06] border border-cyan-500/[0.1]"><span className="text-[10px] text-white/30 uppercase tracking-wider">This Month</span><p className="text-lg font-semibold text-cyan-400 mt-1">{formatCurrency(stats.monthlyRevenue)}</p></div>
                ...
                <div className="flex items-center justify-center py-4">
                  <div className="relative h-28 w-28">
                    <svg className="h-28 w-28 -rotate-90" viewBox="0 0 120 120">
                      ...
                      <circle cx="60" cy="60" r="50" fill="none" stroke="url(#gradient)" strokeWidth="10" strokeDasharray={`${stats.successRate * 3.14} ${314 - stats.successRate * 3.14}`} strokeLinecap="round" />
